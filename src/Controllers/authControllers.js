    ...
    try {
        ...
        try {
            ...
            if (updateuser.modifiedCount === 0) {
                return res.status(404).json({ message: 'No se encontró el usuario o no se realizó ningún cambio.' });
            }
            return res.status(200).json({message:'Imagen subida y guardada', file: req.file.path});
        } catch (error) {
